...
Versie 20 corrigeert de belangrijkste resterende vertekening in de kwantitatieve laag. Correlatie en Markowitz gebruiken niet langer alle kalenderdagen, waarbij een doorgetrokken vlak aandelenweekend tegenover een echte cryptobeweging stond, maar alleen opeenvolgende datums waarop alle betrokken assets werkelijk zijn waargenomen. Minder dan 60 gezamenlijke rendementsintervallen blokkeert de uitkomst; steekproefperiode, omvang en afgeleide periodes-per-jaar zijn zichtbaar. Assetwegingen, cashweging, diversificatie, alerts en stressverlies gebruiken nu dezelfde totale-portefeuillewaarde. Cash blijft ongewijzigd onder een stressscenario en blijft bewust buiten de risky-asset-frontier. Dashboardafleidingen hergebruiken daarnaast één expliciete 1.095-daagse ledger in plaats van die meerdere keren per render opnieuw op te bouwen.

Versie 21 maakt de gebruikers- en releasegrenzen reproduceerbaar. Vijf logische Playwright-scenario's lopen in Chromium, Firefox en WebKit met synthetische data en bewaken de lege staat, fail-closed importpreview, toetsenbordfocus, schema-2/3/4-restore, kwantitatieve weergave, service-workerupgrade en de afwezigheid van onverwachte externe requests. De matrix legde een WebKit-specifiek focusverlies via de systeembestandskiezer en muisklikken bloot; modals bewaren daarom nu expliciet hun activerende element. CI heeft afzonderlijke verplichte checks voor de Node-suite en de browsermatrix. GitHub Pages bouwt niet langer vanuit de repository-root, maar publiceert uitsluitend een opnieuw gevalideerde `dist/` met 15 individueel goedgekeurde runtimebestanden. `main` vereist beide actuele checks en opgeloste reviewgesprekken; force-push en branchverwijdering zijn uitgeschakeld.
...
| P1 | Modals misten Escape, focuslus en dialoogsemantiek | ARIA, Escape, focus trap, focusherstel en live toast; activerende knoppen worden expliciet doorgegeven omdat WebKit een geklikte knop niet altijd focust | Afgerond voor transactie-, importpreview- en opdrachtmodal |
| P1 | Geen tests of CI | 59 Node-tests, publieke-buildvalidator en afzonderlijke GitHub Actions-checks voor regressie en browser-E2E | Afgerond |
| P1 | Kritieke gebruikersflows en schemaherstel waren niet in echte browsers getest | Synthetische Playwright-flows voor lege staat, preview zonder mutatie, schema 2/3/4, toetsenbord, analyses, netwerkgrens en service-workerupgrade in Chromium, Firefox en WebKit | Afgerond in versie 21 |
| P1 | Legacy Pages publiceerde vanuit de repository-root en `main` had geen verplichte kwaliteitsgrens | Fail-closed artifact met 15 expliciete runtimebestanden; Pages uploadt alleen `dist/`; `test` en `browser-e2e` zijn verplichte actuele checks en force-push/verwijderen zijn geblokkeerd | Afgerond in versie 21 |
| P1 | Kalenderweekenden verlaagden gemengde aandelen/cryptocorrelaties kunstmatig en voedden de frontier met ongelijke observaties | Correlatie en frontier gebruiken gezamenlijke waargenomen intervallen, minimaal 60 samples en annualisatie op feitelijke observatiefrequentie; UI toont de sample | Afgerond in versie 20 |
| P1 | Inzichten, wegingalerts en stress gebruikten verschillende noemers en stress negeerde cash | Centrale allocatieberekening inclusief cash; HHI bevat cash als bucket, alerts delen dezelfde noemer en cash blijft ongewijzigd in stress | Afgerond in versie 20 |
| P2 | Eén dashboardrender bouwde de volledige 1.095-daagse ledger herhaaldelijk op | Portefeuille, posities, externe flows, TWR, dag-P&L en alerts kunnen dezelfde expliciete ledger hergebruiken; regressietest bewaakt één build | Afgerond in versie 20 |
| P2 | Dynamische assetnamen in frontieradvies en AI-observaties vertrouwden alleen op ingestnormalisatie | Dynamische namen en signaallabels worden ook op het uiteindelijke `innerHTML`-renderpunt ge-escaped | Afgerond in versie 20 |
...
Positief zijn de nul-backendarchitectuur, CSP, opt-in netwerkgrens, afwezigheid van runtime-dependencies, gesloten valutafouten, maximale importgroottes en gevalideerde opslagobjecten. De publieke-buildvalidator controleert dat bekende privépaden niet onder versiebeheer staan en de deployment kopieert alleen individueel toegestane runtimebestanden naar een schone artifactmap.
...
De Node-regressiesuite telt na versie 21 59 tests en controleert onder meer v3→v4-transactiemigratie, interne herbalancering, fees/dividend/kostbasis, realized P&L, splits, transfers, oversell-beveiliging, reconciliatie, XIRR en schema-2/3/4-backupherstel. Datumregressies starten dezelfde marktdata op een latere systeemdatum, controleren dat de bronobservatie op haar oorspronkelijke kalenderdag blijft staan, bewaken het 1/4-daagse actualiteitsvenster en toetsen kalenderdagverschillen over zomer- en wintertijd. Toekomstige transacties worden zowel bij import als in de ledger fail-closed getest. Importtests bewijzen dat JSON-, CSV- en backuppreviews niets muteren en dekken onbekende richting, NL/EN-DEGIRO met veilige FX, Bitvavo-funding over losse exports, crypto-fees, transferkwaliteit, stakingrewards en dedupe. Kwantitatieve tests bewaken gezamenlijke waargenomen intervallen, minimale samplegrootte, marktfrequentie, cashweging, cashgedempte stress en één ledgerbuild voor gedeelde afleidingen. Daarnaast blijven netwerkgrenzen, DCA zonder look-ahead, walk-forward-arena, syntaxis en statische security-/cache-eisen gedekt.

Playwright voegt vijf logische gebruikersscenario's toe. Over Chromium, Firefox en WebKit worden 13 tests uitgevoerd; de service-workerupgrade draait bewust alleen in Chromium en veroorzaakt de twee verwachte skips. De tests gebruiken uitsluitend synthetische fixtures, melden iedere page-/console-exception en falen op een externe netwerkrequest terwijl toestemming uitstaat. Ze bewijzen tevens dat schema-4-data na reload reconciliatie, frontier, correlatie en cashstress rendert.

GitHub Actions levert twee gescheiden statuschecks: `test` voor Node en publicatievalidatie, en `browser-e2e` voor de drie browsers. Branch protection vereist beide checks op de actuele `main`, vereist opgeloste gesprekken en lineaire geschiedenis en verbiedt force-push en verwijderen. De Pages-workflow herhaalt `npm run check`, bouwt daarna een schoon artifact via een expliciete bestandsallowlist en uploadt uitsluitend `dist/`. De live deployment blijft statisch. Productie-securityheaders zijn nog steeds een hostingverantwoordelijkheid; GitHub Pages biedt daar geen projectspecifieke controle over.

## Vervolgplan

1. **P1 — instrumentidentiteit:** ISIN + beurs + quotevaluta als primaire sleutel; ticker alleen als label/zoekterm. Voeg gecontroleerde symboolmigraties toe zonder historische breuk.
2. **P1 — fiscale lots en corporate actions:** optionele FIFO/lots, return-of-capital, fusies en spin-offs met expliciete kostbasisregels.
3. **P2 — multi-currency ledger:** afzonderlijke cashrekeningen en gevalideerde historische FX-bronnen in plaats van één handmatige rate per event.
4. **P2 — opslagmigratie en modulegrenzen:** IndexedDB met migraties/checksums, optionele versleutelde backup, ES-modules en expliciete provideradapters.
5. **P2 — productiehardening en herstelproef:** eigen hosting met securityheaders, periodieke live-smokechecks, providercontracttests en een gedocumenteerde backup-/restoreoefening.

Browser-E2E, een fail-closed Pages-artifact en de verplichte mergegrens zijn nu uitgevoerd. De eerstvolgende investering hoort instrumentidentiteit te zijn; dat vermindert het risico op een correcte berekening voor het verkeerde effect meer dan verdere verfijning van ML.
